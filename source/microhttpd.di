 ...
 * creating a response).
 */
enum MHD_SIZE_UNKNOWN = UINT64_MAX;

enum MHD_CONTENT_READER_END_OF_STREAM = SIZE_MAX;
enum MHD_CONTENT_READER_END_WITH_ERROR = SIZE_MAX - 1;
...
/* !defined(_WIN32) || defined(_SYS_TYPES_FD_SET) */

/* !defined(_WIN32) || defined(_SYS_TYPES_FD_SET) */
enum MHD_SOCKET_DEFINED = 1;
/* MHD_SOCKET_DEFINED */
 ...
 */
enum MHD_LONG_LONG_PRINTF = "ll";
enum MHD_UNSIGNED_LONG_LONG_PRINTF = "%llu";
 ...
 * @ingroup specialized
 */
enum MHD_ICY_FLAG = cast(uint) (cast(uint) 1) << 31;
 ...
 */
alias MHD_AcceptPolicyCallback = int function (
    void* cls,
    const(sockaddr)* addr,
    socklen_t addrlen);
 ...
 */
alias MHD_AccessHandlerCallback = int function (
    void* cls,
 ...
 * @ingroup request
 */
alias MHD_KeyValueIterator = int function (
    void* cls,
    MHD_ValueKind kind,
    const(char)* key,
    const(char)* value);
 ...
 */
alias MHD_ContentReaderCallback = c_long function (
    void* cls,
    ulong pos,
    char* buf,
    size_t max);
 ...
 *         #MHD_NO to abort the iteration
 */
alias MHD_PostDataIterator = int function (
    void* cls,
    MHD_ValueKind kind,
    const(char)* key,
    const(char)* filename,
 ...
 * @ingroup event
 */
MHD_Daemon* MHD_start_daemon_va (
    uint flags,
    ushort port,
 ...
 * @ingroup event
 */
extern (D) auto MHD_get_fdset(T0, T1, T2, T3, T4)(auto ref T0 daemon, auto ref T1 read_fd_set, auto ref T2 write_fd_set, auto ref T3 except_fd_set, auto ref T4 max_fd)
{
    return MHD_get_fdset2(daemon, read_fd_set, write_fd_set, except_fd_set, max_fd, FD_SETSIZE);
}
 ...
 * @ingroup event
 */
int MHD_get_timeout (MHD_Daemon* daemon, ulong* timeout);
 ...
 * @ingroup response
 */
MHD_Response* MHD_create_response_from_data (
    size_t size,
    void* data,
    int must_free,
    int must_copy);
 ...
 * @ingroup response
 */
MHD_Response* MHD_create_response_from_fd64 (ulong size, int fd);
 ...
 * @ingroup response
 */
MHD_Response* MHD_create_response_from_fd_at_offset (
    size_t size,
    int fd,
    off_t offset);
 ...
 * @ingroup response
 */
MHD_Response* MHD_create_response_from_fd_at_offset64 (
    ulong size,
    int fd,
    ulong offset);
 ...
 * @ingroup response
 */
const(char)* MHD_get_response_header (MHD_Response* response, const(char)* key);
 ...
 * @ingroup request
 */
MHD_PostProcessor* MHD_create_post_processor (
    MHD_Connection* connection,
    size_t buffer_size,
 ...
 * @ingroup request
 */
int MHD_destroy_post_processor (MHD_PostProcessor* pp);
 ...
 * @ingroup authentication
 */
int MHD_queue_auth_fail_response (
    MHD_Connection* connection,
    const(char)* realm,
    const(char)* opaque,
    MHD_Response* response,
 ...
 * @ingroup authentication
 */
char* MHD_basic_auth_get_username_password (
    MHD_Connection* connection,
    char** password);
 ...
 * @ingroup authentication
 */
int MHD_queue_basic_auth_fail_response (
    MHD_Connection* connection,
    const(char)* realm,
    MHD_Response* response);
 ...
 * @ingroup specialized
 */
const(MHD_ConnectionInfo)* MHD_get_connection_info (
    MHD_Connection* connection,
 ...
 * @ingroup specialized
 */
const(MHD_DaemonInfo)* MHD_get_daemon_info (
    MHD_Daemon* daemon,
 ...
 * @ingroup specialized
 */
int MHD_is_feature_supported (MHD_FEATURE feature);
